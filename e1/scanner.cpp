...
void Scanner::ensureLookahead(size_t k) {
    ...
    while (la_.size() < k) {
        ...
        if (!t || t->type == Token::END || t->type == Token::ERR) break;
    }
}

Token* Scanner::peekToken(int k) {
    if (k <= 0) k = 1;
    ensureLookahead(static_cast<size_t>(k));
    if (la_.size() < static_cast<size_t>(k)) return nullptr;
    return la_[static_cast<size_t>(k) - 1]; 
}
...
Token* Scanner::nextToken() {
    ...
    // ID
        ...
        else if (lexema=="cap") return new Token(Token::CAP, input, first, current - first);
        else if (lexema=="cup") return new Token(Token::CUP, input, first, current - first);
    ...
    // Operadores
        ...
        switch (c) {
            ...
            case '\\': token = new Token(Token::BACK, c); break;
